...
Codex 额度小组件是一个桌面悬浮工具。它通过本机已登录的 Codex 读取额度信息，并用面板或悬浮球展示周额度、上周与本周额度估算、剩余重置次数、刷新时间和重置时间。

### 功能介绍

- 面板模式：仪表固定展示周额度，并展示上周与本周额度估算、剩余重置次数、刷新时间和重置时间。
- 悬浮球模式：用更小窗口展示周额度，适合长期放在桌面边缘。
...
仅同时满足以下条件才展示金额：有效百分点样本至少 `10` 个、唯一覆盖跨度至少 `5%`、斜率为正且有限。模型组合、长上下文比例、缓存命中和样本分布变化都会使估算值发生变化。
