...
应用首次流式扫描 `CODEX_HOME/sessions` 和 `CODEX_HOME/archived_sessions` 最近 16 天的本地会话日志，后续刷新在进程内复用未变化的 rollout。文件增长时会流式重读整个旧前缀并校验内容摘要，一致时只解析新增部分；因此增量刷新仍有旧内容的读取开销，但省去重复解析。文件被截断、重写或尾行不完整时自动完整重读。读取失败或提前结束时不提交不完整缓存。摘要仅用于检测普通日志重写，不作为安全认证机制。

扫描只提取模型、Token 用量、周额度百分比和重置时间，跨文件事件排序保留亚秒时间精度。同一 rollout 在归档移动期间出现重复副本时只读取较新的文件。累计 Token 指纹用于文件内费用去重：首次有效事件计费一次，缺少有效时间或周额度的记录不占用指纹；同一用量后补的百分比或重置时间变化仍保留，已计价用量不重复计费，原费用未知时仍保持未知。完全重复的额度记录忽略。

周窗口按 `10080` 分钟识别；30 分钟内漂移的重置时间视为同一周期，当前周期允许与实时重置时间相差 2 小时，上周期取此前最近的有效周期。本周期尚无本地事件时，仍可展示上周期估值；此时候选重置时间须早于实时重置时间超过 2 小时，再按最近活动时间选择。
