...
export function mount(root) {
  ...
  meter.className = "basic2-meter";
  const svg = svgElement("svg", { class: "basic2-gauge", viewBox: "0 0 130 130", role: "img" });
  const progress = svgElement("circle", {
    class: "basic2-gauge-progress",
    cx: 65,
    cy: 65,
    r: 43,
    pathLength: 100,
  ...
  const outerProgress = svgElement("circle", {
    class: "basic2-gauge-outer-progress",
    cx: 65,
    cy: 65,
    r: 51,
    pathLength: 100,
  ...
  const inner = svgElement("g", { class: "basic2-gauge-inner" });
  const mark = svgElement("path", { class: "basic2-gauge-mark", transform: MARK_TRANSFORM, d: MARK_PATH });
  const percent = svgElement("text", { class: "basic2-gauge-percent", x: 65, y: 96, "text-anchor": "middle" });
  const label = svgElement("text", { class: "basic2-gauge-label", x: 65, y: 111, "text-anchor": "middle" });
  inner.append(
    svgElement("circle", { class: "basic2-gauge-track", cx: 65, cy: 65, r: 43, pathLength: 100 }),
    progress,
    mark,
    percent,
    label
  ...
  svg.append(
    createDefs(),
    svgElement("circle", { class: "basic2-gauge-glow", cx: 65, cy: 65, r: 58 }),
    svgElement("circle", { class: "basic2-gauge-sphere", cx: 65, cy: 65, r: 56 }),
    svgElement("path", { class: "basic2-gauge-sheen", d: "M24 59c4-22 20-38 43-42 17-3 31 2 41 11-13-3-30-2-47 6-17 8-29 18-37 25Z" }),
    svgElement("circle", { class: "basic2-gauge-outer-track", cx: 65, cy: 65, r: 51, pathLength: 100 }),
    outerProgress,
    inner
  );
  meter.append(svg);
  ...
  function update({ percent: nextPercent, angle = 0, level, label: nextLabel, mode = "panel", dock = "none" }) {
    ...
    meter.style.setProperty("--remaining-angle", `${angle}deg`);
...
function createDefs() {
  ...
  const sphere = svgElement("radialGradient", { id: "basic2GaugeSphere", cx: "25%", cy: "0%", r: "92%" });
  sphere.append(
    svgElement("stop", { offset: "0%", class: "basic2-gauge-sphere-stop-a" }),
    svgElement("stop", { offset: "44%", class: "basic2-gauge-sphere-stop-b" }),
    svgElement("stop", { offset: "100%", class: "basic2-gauge-sphere-stop-c" })
  );
  const accent = svgElement("linearGradient", { id: "basic2GaugeAccent", x1: "22", y1: "18", x2: "108", y2: "112" });
  accent.append(
    svgElement("stop", { offset: "0%", class: "basic2-gauge-accent-stop-a" }),
    svgElement("stop", { offset: "100%", class: "basic2-gauge-accent-stop-b" })
  );
